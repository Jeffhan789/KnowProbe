...
def show_config(
    ...
    format: Annotated[
        str,
        typer.Option("--format", "-f", help="Output format (table|yaml|json)", case_sensitive=False),
    ] = "table",
...
def validate_config(
    ...
    try:
        ...
        print_info(f"Parsed YAML successfully")
        ...
        if env_vars:
            ...
            for var, value in env_vars.items():
                ...
                display_value = "***" if "key" in var.lower() or "secret" in var.lower() or "password" in var.lower() else value
...
@app.command("env")
def show_env(
    verbose: Annotated[bool, typer.Option("--verbose", "-v", help="Show all env vars, not just KNOWPROBE_")] = False,
) -> None:
...
@app.command("init")
def init_config(
    output: Annotated[Path, typer.Option("--output", "-o", help="Output config file path")] = Path("configs/local.yaml"),
...
def _check_environment_variables() -> dict[str, str]:
    ...
    for key, value in os.environ.items():
        if key.startswith("KNOWPROBE_") or key in ("OPENAI_API_KEY", "DEEPSEEK_API_KEY", "ANTHROPIC_API_KEY"):
            relevant[key] = value
    return relevant
...
def _display_config_warnings(settings) -> None:
    ...
    if settings.models.local.timeout < 30:
        warnings.append(f"Model timeout ({settings.models.local.timeout}s) may be too short for large models")
